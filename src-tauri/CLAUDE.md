...
| Workspace domain split | Low-dependency backend modules and pure entities move into `src-tauri/crates/ralphx-domain`; shared memory/team types belong there, while Tauri/SQLite-facing or root-coupled code stays in the root crate until a clean boundary exists |
