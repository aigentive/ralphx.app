...
| Completion shutdown grace | After `execution_complete` / `complete_review` / `complete_merge`, stream timeout logic must honor `completion_grace_secs` and treat post-completion non-zero exits as successful shutdowns |
