...
impl GitService {
    ...
    /// Attempt a merge in an isolated worktree (Worktree mode merge isolation).
    ///
    /// Creates a temporary worktree at `merge_worktree_path` on `target_branch`,
    /// then merges `source_branch` into it. The main repo is never checked out.
    ///
    /// On success: the merge commit is on `target_branch` in the worktree.
    /// On conflict: merge is aborted, worktree is left intact for the merger agent.
    ///
    /// # Arguments
    /// * `repo` - Path to the main git repository
    /// * `source_branch` - Branch to merge (e.g., task branch)
    /// * `target_branch` - Branch to merge into (e.g., feature branch or main)
    /// * `merge_worktree_path` - Where to create the temporary merge worktree
    pub fn try_merge_in_worktree(
        ...
        debug!(
            "Attempting merge of '{}' into '{}' in worktree {:?} (repo: {:?})",
            source_branch, target_branch, merge_worktree_path, repo
        );
        ...
        if let Some(parent) = merge_worktree_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                AppError::GitOperation(format!(
                    "Failed to create merge worktree parent directory {:?}: {}",
                    parent, e
                ))
            })?;
        }

        // Step 3: Create worktree on target branch (no new branch — checkout existing)
        let output = Command::new("git")
            .args([
                "worktree",
                "add",
                merge_worktree_path.to_str().unwrap_or_default(),
                target_branch,
            ])
            .current_dir(repo)
            .output()
            .map_err(|e| {
                AppError::GitOperation(format!("Failed to create merge worktree: {}", e))
            })?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(AppError::GitOperation(format!(
                "Failed to create merge worktree at {:?}: {}",
                merge_worktree_path, stderr
            )));
        }

        // Step 4: Merge source branch into target in the worktree
        let merge_output = Command::new("git")
            .args(["merge", source_branch, "--no-edit"])
            .current_dir(merge_worktree_path)
            .output()
            .map_err(|e| {
                AppError::GitOperation(format!("Failed to run git merge in worktree: {}", e))
            })?;

        if merge_output.status.success() {
            let commit_sha = Self::get_head_sha(merge_worktree_path)?;
            debug!(
                "Merge in worktree succeeded for '{}' into '{}', SHA: {}",
            ...
            || stdout.contains("conflict") || stderr.contains("conflict")
        {
            ...
            // Abort merge but keep the worktree — the merger agent will use it
            Self::abort_merge(merge_worktree_path)?;
            debug!(
                "Merge in worktree conflict for '{}' into '{}', files: {:?}",
                source_branch, target_branch, conflict_files
            );
    ...
    // =========================================================================
    // Worktree-Isolated Merge Operations
    // =========================================================================
