...
fn test_user_hint_factor_multiple_hints() {
    let service = create_service(vec![], vec![]);
    let factor =
        service.calculate_user_hint_factor("Do this immediately, it's a blocker and urgent");
...
async fn test_assess_and_update_all_priorities() {
    ...
    let proposal_repo = Arc::new(MockTaskProposalRepository::with_proposals(vec![
        p1.clone(),
        p2.clone(),
    ]));
