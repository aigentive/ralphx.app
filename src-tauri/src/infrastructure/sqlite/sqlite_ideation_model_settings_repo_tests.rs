...
async fn upsert_and_get_global_row() {
    ...
    // Upsert global row
    let result = repo.upsert_global("sonnet", "opus").await.unwrap();
    ...
    // Update it
    let updated = repo.upsert_global("haiku", "inherit").await.unwrap();
...
async fn upsert_and_get_project_row() {
    ...
    let result = repo
        .upsert_for_project(project_id, "opus", "inherit")
        .await
        .unwrap();
...
async fn upsert_is_idempotent() {
    ...
    let first = repo.upsert_global("sonnet", "opus").await.unwrap();
    let second = repo.upsert_global("sonnet", "opus").await.unwrap();
    ...
    let proj_first = repo
        .upsert_for_project("proj-1", "haiku", "inherit")
        .await
        .unwrap();
...
async fn upsert_updates_existing_row() {
    ...
    // Insert global
    repo.upsert_global("sonnet", "sonnet").await.unwrap();
    // Update global
    let updated = repo.upsert_global("opus", "haiku").await.unwrap();
...
async fn global_and_project_rows_are_independent() {
    ...
    repo.upsert_for_project("proj-1", "haiku", "inherit")
        .await
        .unwrap();
