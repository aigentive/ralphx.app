...
//
// Tracks whether the session title was set by the ralphx-utility-session-namer agent ('auto')
// or by the user via manual rename ('user'). Used to decide whether to re-trigger
// the ralphx-utility-session-namer at plan acceptance time.
...
pub fn migrate(conn: &Connection) -> AppResult<()> {
    // title_source: 'auto' (set by ralphx-utility-session-namer) | 'user' (set by manual rename)
