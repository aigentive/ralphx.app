// Tests for v22_plan_selection_stats migration

use super::v22_plan_selection_stats;
use crate::infrastructure::sqlite::open_memory_connection;

#[test]
fn test_migration_v22_creates_plan_selection_stats_table() {
    let conn = open_memory_connection().unwrap();

    // Run migration
    v22_plan_selection_stats::migrate(&conn).unwrap();
...
#[test]
fn test_migration_v22_creates_indexes() {
    let conn = open_memory_connection().unwrap();
...
#[test]
fn test_migration_v22_is_idempotent() {
    let conn = open_memory_connection().unwrap();
