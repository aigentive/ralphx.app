...
impl PlanSelectionStatsRepository for SqlitePlanSelectionStatsRepository {
    ...
    async fn get_stats_batch(
        ...
        let session_id_strs: Vec<String> = session_ids.iter().map(|id| id.as_str().to_string()).collect();
...
mod tests {
    ...
    fn setup_test_db() -> (SqlitePlanSelectionStatsRepository, ProjectId, IdeationSessionId) {
        ...
        (SqlitePlanSelectionStatsRepository::new(conn), project_id, session_id)
    }
