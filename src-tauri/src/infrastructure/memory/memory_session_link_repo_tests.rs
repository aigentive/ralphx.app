...
async fn test_create_link() {
    let repo = MemorySessionLinkRepository::new();
...
#[tokio::test]
async fn test_get_by_parent_empty() {
    let repo = MemorySessionLinkRepository::new();
    let parent_id = IdeationSessionId::new();
...
#[tokio::test]
async fn test_get_by_child_empty() {
    let repo = MemorySessionLinkRepository::new();
    let child_id = IdeationSessionId::new();
...
#[tokio::test]
async fn test_delete_by_child_preserves_other_children() {
    let repo = MemorySessionLinkRepository::new();
    let parent_id = IdeationSessionId::new();
    let child_id1 = IdeationSessionId::new();
    let child_id2 = IdeationSessionId::new();
    ...
    // Delete links for child_id1
    repo.delete_by_child(&child_id1)
        .await
        .expect("Failed to delete");

    // Verify child_id1 links are gone but child_id2 links remain
    let child1_links = repo
        .get_by_child(&child_id1)
        .await
        .expect("Failed to query");
    assert_eq!(child1_links.len(), 0);

    let child2_links = repo
        .get_by_child(&child_id2)
        .await
        .expect("Failed to query");
    assert_eq!(child2_links.len(), 1);
}
