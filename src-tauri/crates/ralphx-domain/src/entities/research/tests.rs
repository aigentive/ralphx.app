...
fn research_brief_with_context() {
    let brief = ResearchBrief::new("Question").with_context("Context info");
    assert_eq!(brief.context, Some("Context info".to_string()));
}

#[test]
fn research_brief_with_scope() {
    let brief = ResearchBrief::new("Question").with_scope("Backend only");
...
fn research_brief_with_constraints() {
    let brief = ResearchBrief::new("Question").with_constraints(["Constraint 1", "Constraint 2"]);
    assert_eq!(brief.constraints.len(), 2);
}
...
fn research_output_serializes() {
    let output = ResearchOutput::new("bucket").with_artifact_type(ArtifactType::Findings);
...
fn research_process_with_preset() {
    let brief = ResearchBrief::new("Question");
    let process =
        ResearchProcess::new("Test", brief, "agent").with_preset(ResearchDepthPreset::Exhaustive);
    let resolved = process.resolved_depth();
...
fn research_process_with_output() {
    let brief = ResearchBrief::new("Question");
    let output = ResearchOutput::new("custom-bucket").with_artifact_type(ArtifactType::Findings);
    let process = ResearchProcess::new("Test", brief, "agent").with_output(output);
...
fn research_process_lifecycle() {
    let brief = ResearchBrief::new("Question");
    let mut process =
        ResearchProcess::new("Test", brief, "agent").with_preset(ResearchDepthPreset::QuickScan);
...
fn research_process_progress_percentage() {
    let brief = ResearchBrief::new("Question");
    let mut process =
        ResearchProcess::new("Test", brief, "agent").with_preset(ResearchDepthPreset::QuickScan); // 10 max iterations
...
fn research_process_should_checkpoint() {
    let brief = ResearchBrief::new("Question");
    let mut process =
        ResearchProcess::new("Test", brief, "agent").with_preset(ResearchDepthPreset::QuickScan); // checkpoint_interval = 5
