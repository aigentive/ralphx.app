use crate::entities::{IdeationSessionBuilder, ProjectId, SessionOrigin, SessionPurpose, TaskId};

use super::{build_child_session, matching_blocker_followup_session, resolve_child_origin, ChildSessionDraftInput};
...
fn sample_parent() -> crate::entities::IdeationSession {
    let mut parent = IdeationSessionBuilder::new()
        ...
        .build();
    parent.plan_artifact_id = Some(crate::entities::ArtifactId::from_string("artifact-1".to_string()));
    parent
}
...
fn matching_blocker_followup_session_ignores_archived_or_mismatched_children() {
    ...
    let matched = matching_blocker_followup_session(
        &[archived, live.clone()],
        "task-1",
        "fingerprint-1",
    )
...
fn build_child_session_inherits_expected_parent_context() {
    ...
    assert_eq!(child.source_task_id.as_ref().map(|id| id.as_str()), Some("task-1"));
