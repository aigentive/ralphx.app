...
function buildUsageTotals(messages: ChatMessageResponse[]) {
  return messages.reduce(
    (totals, message) => ({
      ...
      cacheCreationTokens: totals.cacheCreationTokens + (message.cacheCreationTokens ?? 0),
...
function buildUsageBuckets(
  ...
  return Array.from(buckets.entries())
    ...
    .sort((a, b) => b.usage.inputTokens - a.usage.inputTokens || b.count - a.count || a.key.localeCompare(b.key));
}
...
export function buildFallbackConversationStats(
  ...
  return {
    ...
    usageCoverage: {
      ...
      runCount: 0,
      runsWithUsage: 0,
      effectiveTotalsSource: providerMessagesWithUsage.length > 0 ? "messages" : "none",
    },
    attributionCoverage: {
      providerMessageCount: providerMessages.length,
    ...
    byUpstreamProvider: buildUsageBuckets(
      providerMessagesWithUsage,
      (message) => message.upstreamProvider ?? conversation.upstreamProvider ?? null,
    ),
    byModel: buildUsageBuckets(providerMessagesWithUsage, (message) => message.effectiveModelId ?? null),
