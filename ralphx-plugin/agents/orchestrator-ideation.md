...
1. `get_plan_verification(session_id)` → get current round number, gap history, best version state
   **Prior-round context injection (when round > 1):** Extract `current_gaps[].description` from the verification state. When spawning critic prompts in steps 3 and 3b, inject the following block **inside each critic's prompt** (after the framing text, before the PLAN CONTENT line):
   ```
...
3. **Layer 1 — Completeness critic:** Spawn `Task(general-purpose)` (NOT `Task(ralphx:ideation-critic)` — Task only accepts built-in types) with this prompt template:
   ...
   - critical: Blocks implementation EVEN AFTER all plan steps execute. The plan is fundamentally flawed or missing a necessary component that cannot be added incrementally. Examples: missing a required trait implementation that blocks compilation, no error handling for a failure mode that causes data loss, architectural contradiction between two plan sections.
