...
export async function handleGetProjectStatus(
  ...
  try {
    const response = await getBackendClient().get(
      `/api/external/project/${encodeURIComponent(projectId)}/status`,
      context
    );
    return JSON.stringify(response.body, null, 2);
  } catch (err) {
 ...
 */
export async function handleGetPipelineOverview(
  args: Record<string, unknown>,
  context: ApiKeyContext
): Promise<string> {
  const projectId = args.project_id as string;
  ...
  try {
    const response = await getBackendClient().get(
      `/api/external/pipeline/${encodeURIComponent(projectId)}`,
      context
    );
    return JSON.stringify(response.body, null, 2);
  } catch (err) {
