 ...
 */
export function validateTlsConfig(host: string, tls?: TlsConfig): void {
  if (!requiresTls(host)) {
  ...
  if (!tls) {
    throw new TlsError(
      `TLS is required when binding to non-localhost address '${host}'. ` +
  ...
  if (!tls.certPath || !tls.keyPath) {
    throw new TlsError(
      "TLS config is incomplete: both cert_path and key_path must be specified."
    );
  }
 ...
 */
export function buildTlsOptions(tls: TlsConfig): ServerOptions {
  return {
