 ...
 */
export async function handleGetTaskSteps(args, context) {
    const taskId = args.task_id;
    if (!taskId) {
        return JSON.stringify({ error: "missing_argument", message: "task_id is required" }, null, 2);
    }
    try {
        const response = await getBackendClient().get(`/api/task_steps/${encodeURIComponent(taskId)}`, context);
 ...
 */
export async function handleBatchTaskStatus(args, context) {
    const taskIds = args.task_ids;
    if (!taskIds || !Array.isArray(taskIds) || taskIds.length === 0) {
        return JSON.stringify({ error: "missing_argument", message: "task_ids array is required" }, null, 2);
    }
    try {
        const response = await getBackendClient().post("/api/external/tasks/batch_status", context, { task_ids: taskIds });
        return JSON.stringify(response.body, null, 2);
    }
    catch (err) {
        return handleError(err);
