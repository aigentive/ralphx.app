...
export function parseAllowedToolsFromArgs() {
    for (const arg of process.argv) {
        if (arg.startsWith("--allowed-tools=")) {
            ...
            if (!value)
                return undefined; // empty = fall through
            if (value === "__NONE__")
                return []; // explicit empty sentinel
