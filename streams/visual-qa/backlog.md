...
1. **PermissionDialog** - Uses event bus pattern (permission:request event), should work immediately like AskUserQuestionModal infrastructure. Ready for next iteration.
2. **ProposalEditModal** - Requires navigating to ideation view and clicking edit button on a proposal, OR test-only helper to set editingProposalId state in App.tsx
