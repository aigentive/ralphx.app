...
7. **Lint before commit** (only for what you modified): `src-tauri/` → cargo clippy, `src/` → npm run lint. For Rust tests when using Claude/automation: run **only** `cargo test --lib` (unit tests); full `cargo test` hangs. No `--nocapture`/verbose.
