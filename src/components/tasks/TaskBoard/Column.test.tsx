...
describe("Column", () => {
  ...
  describe("awaiting_merge group category filter", () => {
    const doneGroups = [
      {
        id: "awaiting_merge",
      ...
      },
      {
        id: "completed",
        label: "Completed",
        statuses: ["merged", "approved"] as const,
        canDragFrom: false,
        canDropTo: false,
      },
    ];

    const doneColumn = createMockColumn({ id: "done", name: "Done", mapsTo: "approved" });

    function mockWithTasks(tasks: ReturnType<typeof createMockTask>[]) {
      vi.mocked(useInfiniteTasksQuery).mockReturnValue({
        data: { pages: [{ tasks, total: tasks.length, hasMore: false, offset: 0 }], pageParams: [undefined] } as InfiniteData<TaskListResponse>,
        fetchNextPage: vi.fn(),
        hasNextPage: false,
        isFetchingNextPage: false,
        isLoading: false,
    ...
    it("should show a plan_merge blocked task in the awaiting_merge group", () => {
      const planMergeTask = createMockTask({
        id: "pm-1",
        title: "Plan Merge Task",
        internalStatus: "blocked",
        category: "plan_merge",
      });
      mockWithTasks([planMergeTask]);
      ...
      expect(screen.getByText("Plan Merge Task")).toBeInTheDocument();
      expect(screen.getByText("Awaiting Merge")).toBeInTheDocument();
    });

    it("should exclude a regular blocked task from the awaiting_merge group", () => {
      const regularBlockedTask = createMockTask({
        id: "reg-1",
        title: "Regular Blocked Task",
        internalStatus: "blocked",
        category: "feature",
      });
      mockWithTasks([regularBlockedTask]);
      ...
      // Regular blocked task must not appear in Done at all
      expect(screen.queryByText("Regular Blocked Task")).not.toBeInTheDocument();
      // Group header should not appear either (no tasks → group is not rendered)
      expect(screen.queryByText("Awaiting Merge")).not.toBeInTheDocument();
    });

    it("should show only plan_merge blocked tasks when both categories are present", () => {
      const planMergeTask = createMockTask({
        id: "pm-1",
        title: "Plan Merge Task",
        internalStatus: "blocked",
      ...
      mockWithTasks([planMergeTask, regularBlockedTask]);
    ...
    it("should hide awaiting_merge group when showMergeTasks is false", () => {
      const planMergeTask = createMockTask({
        id: "pm-1",
        title: "Plan Merge Task",
        internalStatus: "blocked",
      ...
      expect(screen.queryByText("Plan Merge Task")).not.toBeInTheDocument();
      expect(screen.queryByText("Awaiting Merge")).not.toBeInTheDocument();
    });
  });
});
