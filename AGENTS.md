...
| P0 | Direct task status mutation audit | Landed more slices: `answer_user_question` now routes through `TaskTransitionService`, `update_task` now rejects status edits so generic task editing can no longer act as a hidden state-transition backdoor, `approve_fix_task` now uses the validated `Blocked -> Ready` transition path, command-side `reject_fix_task` now uses explicit corrective transitions, and `ReviewService` now persists its fix-task/backlog/reject flows through small validated/corrective repo-backed helpers instead of silent raw writes; current classification: `startup_jobs.rs` still does a direct `Escalated -> {PendingReview|Ready|PendingMerge}` crash-recovery write because it batches metadata + recovery target before the later startup phases re-enter the normal machine, merge internals (`merge_completion.rs`, `merge_outcome_handler.rs`, `side_effects/*`) remain the canonical state-machine engine with their own freshness checks/history/events, and the remaining review-side raw writes are limited to task creation/bootstrap defaults for new fix tasks; next audit whether any other production helpers still mutate workflow statuses without validated or explicit corrective semantics |
