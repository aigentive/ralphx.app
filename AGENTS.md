...
| P0 | `transition_task*` caller audit and migration | Investigated: the production `transition_task()` / `transition_task_with_metadata()` surface is mapped across review handlers, git/merge handlers, external task actions, chat-service flows, reconciliation, scheduler/execution commands, and project/task commands; landed direct handler coverage for `reviews/human.rs` and `git.rs`, command-side status-guard helpers now lock the Tauri review/git command preconditions in `commands/review_commands.rs` and `commands/git_commands.rs`, merge reconciler coverage now locks the `branch_missing` skip guards, `resume_execution` no longer invents `Executing` on malformed pause metadata because it falls back to real pause history, chat-service completion helpers now lock the runtime targets for execution-complete (`PendingReview` vs `Failed`) and incomplete-review exit (`Escalate` vs shutdown skip vs ignore-already-transitioned), `reconciliation_runner.rs` now locks the completed-execution fallback path that advances `Executing` tasks into `PendingReview`/`Reviewing` when the finished run is discovered via task-execution conversation history rather than state-history metadata, and `pr_poller_tests.rs` now locks both the happy `Merging -> Merged` path and the stale-poller guard that must not regress an already-`Merged` task back to `MergeIncomplete`; next highest-signal remaining validated caller is the deeper merge recovery path in `reconciliation/handlers/merge.rs` |
