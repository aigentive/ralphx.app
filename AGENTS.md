...
| P0 | `transition_task*` caller audit and migration | Investigated: the production `transition_task()` / `transition_task_with_metadata()` surface is mapped across review handlers, git/merge handlers, external task actions, chat-service flows, reconciliation, scheduler/execution commands, and project/task commands; next audit the remaining risky validated callers (`reviews/human.rs`, `git.rs`, `pr_merge_poller.rs`, reconciliation handlers, execution lifecycle, review_commands.rs`) and move any true repair flows onto explicit corrective APIs instead of weakening default validation |
