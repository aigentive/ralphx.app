...
|---|---|---|
| P0 | Global orchestration semantics | Landed: persisted halt mode, explicit stopped UI state, stopped-aware control bar copy, and `resume_execution` rejection after stop; next verify whether any extra user-facing queue messaging is needed |
| P0 | Concurrency admission control | Landed: ideation plus task/review/merge chat spawns now honor project-aware slot admission; next extend the same project-aware admission to scheduler/spawner paths that still rely on the old generic capacity check |
| P0 | Pipeline allocation | Ideation spawn/resume now respect global cap, project ideation cap, project total cap, and borrowing only when execution is not waiting; task/review/merge chat spawns now respect project total caps too; next carry that policy through scheduler-side execution admission |
| P1 | Queue + recovery alignment | Landed: pause queues all slot-consuming sends, resume relaunches paused ideation + active task/review/merge queued work, and stop clears queued slot-consuming work; next decide whether any extra user-facing queue messaging is needed |
...
|---|---|---|
| M1 Admission gate | Landed: global/project ideation admission, verification-child counts-as-ideation tests, borrow-policy tests, and project-aware chat-service admission for task/review/merge; next extend the same project-aware admission into scheduler/spawner paths | `src-tauri/src/application/chat_service/mod.rs`, `src-tauri/src/commands/execution_commands.rs`, targeted tests in `src-tauri/tests/` |
...
| M5 Settings UI | Landed: global/project ideation allocation controls now flow through frontend schemas, API transforms, mocks, `App.tsx`, and `SettingsView`; next validate them against per-project admission and borrowing behavior under real queue pressure | `src/components/settings/SettingsView.tsx`, `src/api/execution.ts`, related schemas/transforms/tests |
