...
| P0 | Direct task status mutation audit | Investigated buckets: `task_commands/mutation.rs` remains the main manual/technical-debt bypass surface (arbitrary `update_task` status edits plus a few restart helpers), `answer_user_question` now routes through `TaskTransitionService`, `startup_jobs.rs` and `review_service.rs` are mostly corrective/bootstrap writes, and merge internals (`merge_completion.rs`, `merge_outcome_handler.rs`, `side_effects/*`) still operate as their own direct state-machine layer; next finish classifying each remaining write as bootstrap, legitimate corrective/internal, or debt that should move behind validated transitions before broadening enforcement |
