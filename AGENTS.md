...
| P0 | Direct task status mutation audit | Landed more slices: `answer_user_question` now routes through `TaskTransitionService`, `update_task` now rejects status edits so generic task editing can no longer act as a hidden state-transition backdoor, `approve_fix_task` now uses the validated `Blocked -> Ready` transition path, and command-side `reject_fix_task` now uses explicit corrective transitions instead of raw writes; current classification: `startup_jobs.rs` raw writes are likely intentional bootstrap/corrective paths that avoid scheduler side effects during recovery, merge internals (`merge_completion.rs`, `merge_outcome_handler.rs`, `side_effects/*`) are the direct state-machine core rather than `transition_task*` callers, and the main remaining technical-debt candidates are the `ReviewService` fix-task helper methods that still perform raw status writes in test/service-only paths; next decide whether to migrate those helpers onto a narrower corrective abstraction or leave them explicitly non-production |
