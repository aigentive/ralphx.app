...
| P0 | Concurrency admission control | Landed: project-aware slot admission now applies across chat spawns, scheduler selection, startup recovery, merge-retry scheduling, manual task resume, paused ideation queue resume fairness across projects, and reconciliation auto-resume/retry paths; next keep expanding mixed-load regression coverage and finish auditing the remaining low-level spawner / diagnostic callers |
| P0 | Pipeline allocation | Ideation spawn/resume now respect global cap, project ideation cap, project total cap, borrowing only when execution is not waiting, and cross-project paused-queue fairness during resume; task/review/merge admission, manual task resume, and reconciliation retries now respect project totals across runtime + recovery paths; next validate under heavier mixed load and decide whether any remaining non-chat spawn path still needs explicit pipeline-aware admission |
