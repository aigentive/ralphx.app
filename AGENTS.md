...
| P0 | Direct task status mutation audit | Landed more slices: `answer_user_question` now routes through `TaskTransitionService`, `update_task` now rejects status edits so generic task editing can no longer act as a hidden state-transition backdoor, and `approve_fix_task` now uses the validated `Blocked -> Ready` transition path; current classification: `startup_jobs.rs` raw writes are likely intentional bootstrap/corrective paths that avoid scheduler side effects during recovery, merge internals (`merge_completion.rs`, `merge_outcome_handler.rs`, `side_effects/*`) are the direct state-machine core rather than `transition_task*` callers, and the main remaining technical-debt candidates are the review fix-task rejection/backlog helper flows; next finish that classification and only then move any remaining debt behind validated or explicit corrective transitions |
