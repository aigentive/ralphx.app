...
| P0 | `transition_task*` caller audit and migration | Investigated: the production `transition_task()` / `transition_task_with_metadata()` surface is mapped across review handlers, git/merge handlers, external task actions, chat-service flows, reconciliation, scheduler/execution commands, and project/task commands; landed direct handler coverage for `reviews/human.rs` and `git.rs`, command-side status-guard helpers now lock the Tauri review/git command preconditions in `commands/review_commands.rs` and `commands/git_commands.rs`, merge reconciler coverage now locks the `branch_missing` skip guards, `resume_execution` no longer invents `Executing` on malformed pause metadata because it falls back to real pause history, and chat-service completion helpers now lock the runtime targets for execution-complete (`PendingReview` vs `Failed`) and incomplete-review exit (`Escalate` vs shutdown skip vs ignore-already-transitioned); next highest-signal remaining validated callers are still the deeper runtime/recovery paths in `chat_service_handlers.rs`, `reconciliation/handlers/execution.rs`, `reconciliation/handlers/merge.rs`, and `application/services/pr_merge_poller.rs` |
