...
| P0 | Concurrency admission control | Landed: project-aware slot admission now applies across chat spawns, scheduler selection, startup recovery, and merge-retry scheduling; next run a broader regression sweep around queue pressure and recovery-heavy flows |
| P0 | Pipeline allocation | Ideation spawn/resume now respect global cap, project ideation cap, project total cap, and borrowing only when execution is not waiting; task/review/merge admission now respects project totals across runtime + recovery paths; next validate under real queue pressure and decide whether any non-chat spawn path still needs explicit pipeline-aware admission |
