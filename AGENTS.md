...
| P0 | `transition_task*` caller audit and migration | Investigated: the production `transition_task()` / `transition_task_with_metadata()` surface is mapped across review handlers, git/merge handlers, external task actions, chat-service flows, reconciliation, scheduler/execution commands, and project/task commands; landed direct handler coverage for `reviews/human.rs` and `git.rs`; next highest-signal remaining validated callers are the Tauri command surfaces in `commands/review_commands.rs` and `commands/git_commands.rs`, then the runtime/recovery paths in `chat_service_handlers.rs`, `execution_commands/lifecycle.rs`, `reconciliation/handlers/execution.rs`, `reconciliation/handlers/merge.rs`, and `application/services/pr_merge_poller.rs` |
...
| P1 | Transition-hardening regression coverage | Landed slices: core `TaskTransitionService` tests now lock `reviewing -> approved` and `merged -> approved` rejection, external `approve_review` regressions remain green, review handler coverage now locks the branchless no-human-review approval path plus human-review `approve_task`/`request_task_changes` entry points, and git handler coverage now locks `complete_merge` idempotence vs invalid-state rejection; next broaden caller-surface regressions only where validated production transitions still lack direct handler/command tests, starting with `commands/review_commands.rs`, `commands/git_commands.rs`, and selected recovery paths that rely on validated transitions rather than canonical engine writes |
