...
| Priority | Stream | Next Step |
|---|---|---|
| P0 | Validated task transitions by default | Investigated: `TaskTransitionService::transition_task*` still persists direct `old_status -> new_status` writes without checking `InternalStatus::can_transition_to`, while product/API callers across reviews, git/merge, external task ops, scheduling, chat-service, reconciliation, and execution lifecycle assume state-machine enforcement; next add the central legality guard in `task_transition_service.rs`, keep same-status no-op behavior, and reject illegal transitions before persistence/history/events/side effects |
| P0 | `transition_task*` caller audit and migration | Investigated: the production `transition_task()` / `transition_task_with_metadata()` surface is mapped across review handlers, git/merge handlers, external task actions, chat-service flows, reconciliation, scheduler/execution commands, and project/task commands; next classify each caller as normal validated workflow vs explicit corrective/recovery path and move true repair flows onto explicit corrective APIs instead of weakening default validation |
...
| P1 | Transition-hardening regression coverage | Planned: add core `TaskTransitionService` tests for illegal jumps like `reviewing -> approved` / `merged -> approved`, keep the external review-action regressions, and preserve corrective-flow tests so recovery semantics remain explicit |
