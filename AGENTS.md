...
| P0 | Model-agnostic MCP/tool UX | Landed: high-friction MCP tools now document parent-vs-child session rules plus concrete payload examples, child-status / session-message tools now show verification-debug usage explicitly, the MCP proxy now appends tool-specific repair hints/examples on backend errors, and the verifier path now has narrower `report_verification_round` / `complete_plan_verification` helpers instead of forcing the generic verification update contract; next extend that repair-oriented treatment to more brittle workflows |
