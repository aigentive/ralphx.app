...
| P1 | Build / compile-coupling reduction | Landed first slice: shared review scope-drift matching + blocker-fingerprint logic now lives in `ralphx-domain`; next keep moving high-churn pure backend decision code out of the heavy `src-tauri` crate so every validation run rebuilds less |
