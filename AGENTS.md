...
| P1 | Settings surface | Backend persistence for ideation allocation landed; next add YAML-seeded defaults and the UI/API controls for global/project ideation caps + borrow toggle |
...
| M4 DB/settings backend | Landed: repo/domain/command/migration coverage for `global_ideation_max`, `project_ideation_max`, `allow_ideation_borrow_idle_execution`; next add YAML-seeded defaults and production bootstrap seeding semantics | execution settings repos/commands, migrations, `src-tauri/ralphx.yaml`, API contracts |
