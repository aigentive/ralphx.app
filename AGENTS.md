...
| P0 | Concurrency admission control | Landed: project-aware slot admission now applies across chat spawns, scheduler selection, startup recovery, merge-retry scheduling, and paused ideation queue resume fairness across projects; next keep expanding mixed-load regression coverage and audit any remaining non-chat spawn paths |
| P0 | Pipeline allocation | Ideation spawn/resume now respect global cap, project ideation cap, project total cap, borrowing only when execution is not waiting, and cross-project paused-queue fairness during resume; task/review/merge admission now respects project totals across runtime + recovery paths; next validate under heavier mixed load and decide whether any non-chat spawn path still needs explicit pipeline-aware admission |
