...
| P0 | Direct task status mutation audit | Landed another slice: `answer_user_question` now routes through `TaskTransitionService`, `update_task` now rejects status edits so generic task editing can no longer act as a hidden state-transition backdoor, `startup_jobs.rs` and `review_service.rs` are still mostly corrective/bootstrap writes, and merge internals (`merge_completion.rs`, `merge_outcome_handler.rs`, `side_effects/*`) still operate as their own direct state-machine layer; next finish classifying each remaining write as bootstrap, legitimate corrective/internal, or debt that should move behind validated transitions before broadening enforcement |
