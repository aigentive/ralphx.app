...
| P0 | Corrective-path separation | Landed more slices: `transition_task_corrective()` now exposes the explicit nonstandard path for command-side repair flows without weakening the default validator, and `reject_fix_task` now uses that corrective path for `Blocked -> Failed` and original-task backlog fallback; next keep documenting that normal workflow callers must not use corrective transitions and audit any remaining raw recovery callers against that boundary |
| P0 | Direct task status mutation audit | Landed more slices: `answer_user_question` now routes through `TaskTransitionService`, `update_task` now rejects status edits so generic task editing can no longer act as a hidden state-transition backdoor, `approve_fix_task` now uses the validated `Blocked -> Ready` transition path, command-side `reject_fix_task` now uses explicit corrective transitions, and `ReviewService` now persists its fix-task/backlog/reject flows through small validated/corrective repo-backed helpers instead of silent raw writes; current classification: `startup_jobs.rs` raw writes are likely intentional bootstrap/corrective paths that avoid scheduler side effects during recovery, merge internals (`merge_completion.rs`, `merge_outcome_handler.rs`, `side_effects/*`) are the direct state-machine core rather than `transition_task*` callers, and the remaining review-side raw writes are limited to task creation/bootstrap defaults for new fix tasks; next keep auditing whether any other production helpers still mutate workflow statuses without validated or explicit corrective semantics |
