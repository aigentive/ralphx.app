...
|---|---|---|
| P0 | Global orchestration semantics | Landed: persisted halt mode, explicit stopped UI state, stopped-aware control bar copy, `resume_execution` rejection after stop, and queued-message pressure surfaced in execution status/control-bar telemetry; next decide whether that telemetry needs any stronger inline warning treatment |
