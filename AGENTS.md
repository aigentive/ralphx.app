...
| P0 | Direct task status mutation audit | Landed more slices: `answer_user_question` now routes through `TaskTransitionService`, `update_task` now rejects status edits so generic task editing can no longer act as a hidden state-transition backdoor, and `approve_fix_task` now uses the validated `Blocked -> Ready` transition path; remaining raw writes are concentrated in `startup_jobs.rs`, `review_service.rs` helper flows, and the merge internals (`merge_completion.rs`, `merge_outcome_handler.rs`, `side_effects/*`); next finish classifying each remaining write as bootstrap, legitimate corrective/internal, or debt that should move behind validated transitions before broadening enforcement |
