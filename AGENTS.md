...
| P1 | Build / compile-coupling reduction | Landed slices: shared review scope-drift matching + blocker-fingerprint logic, `complete_review` tool schema/validation, review follow-up prompt / issue-normalization helpers, review decision/policy validation, review outcome/result shaping, review follow-up / scope-metadata shaping, review issue mapping, review history-selection helpers, merge scope-backstop evaluation, and child-session matching/construction helpers now live in `ralphx-domain`; verification-child creation now reuses that shared child-session path too; next keep moving high-churn pure backend decision code out of the heavy `src-tauri` crate so every validation run rebuilds less |
