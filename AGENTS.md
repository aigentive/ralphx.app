...
| P0 | Concurrency admission control | Landed: project-aware slot admission now applies across chat spawns, scheduler selection, startup recovery, merge-retry scheduling, manual task resume, and paused ideation queue resume fairness across projects; next keep expanding mixed-load regression coverage and finish auditing the remaining old-capacity callers (reconciliation + low-level spawner) |
| P0 | Pipeline allocation | Ideation spawn/resume now respect global cap, project ideation cap, project total cap, borrowing only when execution is not waiting, and cross-project paused-queue fairness during resume; task/review/merge admission plus manual task resume now respect project totals across runtime + recovery paths; next validate under heavier mixed load and decide whether any remaining non-chat spawn path still needs explicit pipeline-aware admission |
