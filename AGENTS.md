...
| P0 | `transition_task*` caller audit and migration | Investigated: the production `transition_task()` / `transition_task_with_metadata()` surface is mapped across review handlers, git/merge handlers, external task actions, chat-service flows, reconciliation, scheduler/execution commands, and project/task commands; landed direct handler coverage for `reviews/human.rs` and `git.rs`, and command-side status-guard helpers now lock the Tauri review/git command preconditions in `commands/review_commands.rs` and `commands/git_commands.rs`; next highest-signal remaining validated callers are the runtime/recovery paths in `chat_service_handlers.rs`, `execution_commands/lifecycle.rs`, `reconciliation/handlers/execution.rs`, `reconciliation/handlers/merge.rs`, and `application/services/pr_merge_poller.rs` |
