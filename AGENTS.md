...
| P1 | Queue + recovery alignment | Startup recovery and runtime ideation queue launches now honor the halt barrier; next make paused ideation actually restart on resume and finalize stop-specific queued-work semantics |
...
| M3 Queue + startup recovery | Partial: runtime ideation sends queue instead of spawning while paused/stopped, and queued continuations stay pending under the halt barrier; next add resume-time ideation re-launch and finish stop-vs-pause queued-work behavior | `src-tauri/src/application/chat_service/mod.rs`, `src-tauri/src/application/chat_service/chat_service_queue.rs`, `src-tauri/src/application/chat_service/chat_service_send_background.rs`, `src-tauri/src/application/recovery_queue.rs`, ideation runtime/external handlers |
