...
| P0 | Harness abstraction | In progress | Landed provider-neutral spawn metadata, Codex exec/resume builders, MCP config override injection, lane-aware queue resume branching, and Codex JSONL stream parsing; next complete unified recovery/reconciliation and consumer normalization |
| P0 | Core milestone | In progress | Ideation lane resolution now permits Codex, send/resume paths branch to Codex `exec` / `exec resume`, and Codex runs operate in explicit solo mode; next validate end-to-end ideation + verification execution and recovery |
...
| P0 | Recovery / capacity / reconciliation | In progress | Provider-neutral session refs now persist on conversations/runs, queue resume branches by harness, interrupted-conversation discovery includes Codex rows, and stale-session recovery/retry keeps the selected harness; next wire startup/reconciliation/execution lifecycle fully off Claude-only assumptions |
...
| P0 | Implementation planning | In progress | Add concrete phase plan, migration plan, regression/test plan, and exact implementation contract docs before touching Rust/frontend runtime code |
| P0 | Phase-1 contract freeze | Started | Lock additive provider-neutral session/run metadata, the new `agent_lane_settings` storage model, normalized event payloads, and the phase-1 Codex solo-lane contract before writing runtime code |
| P0 | Execution backlog | Started | Break the frozen phase-1 contract into PR-sized implementation slices with owned file surfaces, required tests, and explicit out-of-scope items so coding can start without replanning |
