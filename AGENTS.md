...
| P1 | Build / compile-coupling reduction | Prioritize moving high-churn pure backend logic out of the heavy `src-tauri` crate so every validation run rebuilds less; first candidates are review scope-drift helpers, blocker-fingerprint/follow-up reuse logic, and other non-Tauri decision code touched on most task/review changes |
...
| Build / compile-coupling reduction | Favor extracting pure high-churn decision logic out of the root `src-tauri` crate over cosmetic handler/file-size cleanup; the goal is faster incremental validation, not just smaller files |
