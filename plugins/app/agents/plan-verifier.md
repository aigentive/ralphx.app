...
| Affected Files contains ANY of: `side_effects/`, `task_transition_service.rs`, `on_enter_states/`, `chat_service_merge.rs`, `chat_service_streaming.rs` (filename/path-component match, not exact full path) | `ideation-specialist-pipeline-safety` | Affected Files section (per-round parallel dispatch) |
| `task_transition_service.rs` OR `on_enter_states/` in Affected Files, OR change description contains: `pipeline stage`, `new state`, `auto-transition`, `state transition`, `on_enter` | `ideation-specialist-state-machine` | Affected Files + Architecture sections (per-round parallel dispatch) |
