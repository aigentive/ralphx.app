...
export async function completePlanVerificationWithSettlement(deps) {
    ...
    if (settledRound.classification === "complete") {
        ...
        const parsedRequiredFindings = requiredLabels.map((label) => parseTypedVerificationFinding({
            label,
            finding: verificationFindings.find((entry) => entry.critic.trim().toLowerCase() === label),
        }));
        const unusableRequired = parsedRequiredFindings.filter((finding) => !finding.usable);
        if (unusableRequired.length > 0) {
            throw new Error(`Required verification findings were published but unusable: ${unusableRequired
...
//# sourceMappingURL=verification-completion.js.map