...
describe("human-wait", () => {
    ...
    it("treats near-deadline fetch failures as timeouts", () => {
        ...
        expect(isHumanWaitTimeoutError(error, HUMAN_WAIT_CLIENT_TIMEOUT_MS - 5_000, HUMAN_WAIT_CLIENT_TIMEOUT_MS)).toBe(true);
    });
    it("does not treat early fetch failures as timeouts", () => {
        const error = new Error("fetch failed");
        expect(isHumanWaitTimeoutError(error, 30_000, HUMAN_WAIT_CLIENT_TIMEOUT_MS)).toBe(false);
    });
});
//# sourceMappingURL=human-wait.test.js.map