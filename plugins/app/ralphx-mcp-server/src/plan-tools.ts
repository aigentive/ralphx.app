...
export const PLAN_TOOLS: Tool[] = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        caller_session_id: {
          type: "string",
          description:
            "The session ID of the caller. Required when calling from a verification child session to bypass the write lock on the plan artifact.",
        },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      required: ["session_id", "round", "generation"],
    },
  },
  {
    name: "assess_verification_round",
    description:
      "Verifier-oriented helper that classifies whether required critic artifacts are complete, still pending, or an infrastructure failure. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        delegates: {
          type: "array",
          description:
            "Delegated critic/specialist jobs expected to publish current-round artifacts. artifact_prefix is the exact required TeamResearch title prefix.",
          items: {
            type: "object",
            properties: {
              job_id: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: {
          ...
          description:
            "Terminal verification status. Use verified or needs_revision for normal completion, reviewing only for abort/error cleanup, and skipped only for flows where skip is actually allowed.",
        },
        round: {
          type: "integer",
      ...
      required: ["session_id", "status", "generation"],
    },
  },
  {
    name: "update_plan_verification",
    description:
      "Update verification state for an ideation session. Use the PARENT ideation session_id as the canonical target; if a verification child session_id is passed, the backend remaps it automatically. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        convergence_reason: {
          ...
          description:
            "Why verification converged. Required for terminal needs_revision -> verified promotion and recommended on all terminal calls.",
        },
        generation: {
          type: "integer",
          description:
            "Generation counter for zombie protection. Pass on every verifier call. If omitted, weaker models can accidentally write stale state; if mismatched, the server rejects the request.",
        },
      },
