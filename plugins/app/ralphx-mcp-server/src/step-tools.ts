...
export const STEP_TOOLS: Tool[] = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        parent_step_id: {
          type: "string",
          description:
            "Optional: ID of the parent step. Creates a sub-step that tracks a specific coder dispatch. The coder should use get_step_context with this sub-step's ID.",
        },
        scope_context: {
          type: "string",
          description:
            'Optional: JSON string with STRICT SCOPE for this sub-step. Example: \'{"files":["src/foo.ts","src/bar.ts"],"read_only":["src/types.ts"],"instructions":"Implement the caching layer"}\'',
        },
      },
  ...
  },
  {
    ...
    description:
      "Get complete context for a step including parent task, scope constraints, sibling steps, and progress. Coders dispatched by a worker should call this FIRST with their assigned sub-step ID.",
    inputSchema: {
      type: "object",
      properties: {
        step_id: {
  ...
  },
  {
    name: "get_sub_steps",
    description:
      "Get all sub-steps for a parent step. Use this to check progress of coder dispatches after creating sub-steps.",
    inputSchema: {
      type: "object",
      properties: {
        parent_step_id: {
  ...
  },
  {
    ...
    description:
      "Signal that task execution is complete. Call this after all steps are finished to allow the agent process to exit gracefully.",
    inputSchema: {
      type: "object",
      properties: {
        task_id: {
        ...
        test_result: {
          type: "object",
          description:
            "Optional test execution results. Include when tests were run during task execution.",
          properties: {
            tests_ran: {
              type: "boolean",
            ...
            test_summary: {
              type: "string",
              description:
                "Optional human-readable summary of test results (e.g., '42 passed, 0 failed')",
            },
          },
