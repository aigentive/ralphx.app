...
async function runRequiredVerificationCriticRound(args: {
  ...
  const rescueTargets = initialDelegates.filter((delegate) =>
    missingPrefixes.has(delegate.artifact_prefix)
  );
      ...
      const finalAssessment =
        timedOut && rescueBudgetExhausted
          ? {
              ...
              recommended_next_action: "complete_verification_with_infra_failure" as const,
              summary:
...
async function resolveVerifierParentSessionId(
  ...
  if (AGENT_TYPE === "ralphx-plan-verifier" && typeof RALPHX_CONTEXT_ID === "string" && RALPHX_CONTEXT_ID.length > 0) {
    const parentContext = await callTauriGet(`parent_session_context/${RALPHX_CONTEXT_ID}`) as {
      parent_session?: {
        id?: string;
      };
    };
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "get_task_context") {
      ...
      const { session_id: raw_session_id, ...body } = args as {
        session_id?: string;
        round: number;
        gaps?: unknown[];
        generation: number;
      ...
      const isVerifierRoundTerminalUpdate =
        AGENT_TYPE === "ralphx-plan-verifier" &&
      ...
      if (Array.isArray(required_delegates) && required_delegates.length > 0) {
        ...
        settlement = settledRound as Record<string, unknown>;
        ...
        if (isVerifierRoundTerminalUpdate && settledRound.classification === "infra_failure") {
          const infraFailure = await callTauri(
            `ideation/sessions/${session_id}/verification/infra-failure`,
            {
              generation: body.generation,
              convergence_reason:
                body.convergence_reason ?? "agent_error",
              round: body.round,
            }
          );
          result = {
            ...(infraFailure as Record<string, unknown>),
            settlement,
          };
        }
      }
