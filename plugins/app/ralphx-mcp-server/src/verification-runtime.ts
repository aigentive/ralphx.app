import {
  ...
  type VerificationRoundFindingMatch,
...
export function createVerificationRuntime(deps: VerificationRuntimeDeps) {
  ...
  function rememberVerificationRoundState(
    sessionId: string,
    state: CachedVerificationRoundState
  ): void {
    verificationRoundStateBySession.set(sessionId, state);
  }

  function getVerificationRoundState(
    sessionId: string
  ): CachedVerificationRoundState | undefined {
    return verificationRoundStateBySession.get(sessionId);
  }

  function clearVerificationRoundState(sessionId: string): void {
  ...
  async function generateSolutionCritiqueForRound(args: {
  ...
  }): Promise<SolutionCritiqueRoundProjection> {
    if (!args.targetArtifactId) {
      throw new Error(
        "Solution critique enforcement failed: verification round did not include a target artifact id."
      );
    }

    const compiledContext = await callTauri(
      `ideation/sessions/${args.sessionId}/compiled-context`,
      {
    ...
    if (typeof compiledContextArtifactId !== "string") {
      throw new Error(
        "Solution critique enforcement failed: compiled context response did not include artifact_id."
      );
    }

    const critique = await callTauri(
      `ideation/sessions/${args.sessionId}/solution-critique`,
      {
        target_artifact_id: args.targetArtifactId,
    ...
    if (typeof critiqueArtifactId !== "string") {
      throw new Error(
        "Solution critique enforcement failed: critique response did not include artifact_id."
      );
    }
  ...
  function solutionCritiqueInfraFailure(error: unknown): Record<string, unknown> {
  ...
  async function awaitOptionalVerificationDelegates(args: {
    ...
    while (true) {
      ...
      const allSettled = args.delegates.every((delegate) => {
        const findingMatch = findingsByCritic.find(
          (entry) => entry.critic === delegate.critic
        );
    ...
    return {
      ...
      settlement: {
        ...
        missing_required_critics: REQUIRED_VERIFICATION_CRITICS
          .filter(
            (critic) =>
              !args.delegates.some((delegate) => delegate.critic === critic.critic)
          )
  ...
  async function runRequiredVerificationCriticRound(args: {
    ...
    const rescueLaunches = await Promise.all(
      rescueTargets.map(async (target) => {
        const critic = REQUIRED_VERIFICATION_CRITICS.find(
          (entry) => entry.critic === target.critic
        );
        if (!critic) {
          return {
            ok: false as const,
    ...
    const finalDelegates = initialDelegates.map((delegate) => {
      const replacement = successfulRescues.find(
        (rescue) => rescue.critic === delegate.critic
      );
  ...
  async function awaitVerificationRoundSettlement(
    ...
    const uniqueCritics = Array.from(
      new Set(args.delegates.map((delegate) => delegate.critic))
    );
    ...
    while (true) {
      ...
      const findingMatches = await loadVerificationFindingsByCritic({
        ...
        new Set(
          args.delegates
              ...
              .filter((critic): critic is string => Boolean(critic))
        )
      ),
      ...
      const findings_by_critic: VerificationRoundFindingMatch[] = uniqueCritics.map((critic) => {
        ...
        const finalAssessment =
          timedOut && rescueBudgetExhausted
            ? {
                ...
                summary:
                  assessment.missing_required_critics.length > 0
  ...
  async function resolveVerifierParentSessionId(
    ...
    if (agentType === "ralphx-plan-verifier" && typeof contextId === "string" && contextId.length > 0) {
      ...
      throw new Error(
        `${toolName} requires an active ralphx-plan-verifier child session with a resolvable parent ideation session.`
      );
    }
  ...
  async function reportVerificationRoundForTool(args: {
    ...
    const gaps =
      cachedRoundState?.round === body.round
        ? cachedRoundState.mergedGaps
        : body.gaps;
    if (agentType === "ralphx-plan-verifier" && !Array.isArray(gaps)) {
      throw new Error(
        "report_verification_round requires a current backend-owned run_verification_round result for the same round."
      );
    }
  ...
  async function completePlanVerificationForTool(args: {
    ...
    if (
      ...
      throw new Error(
        "complete_plan_verification cannot finalize an actionable needs_revision result without a terminal convergence_reason. Revise the plan, continue the loop, and only finish when the backend-owned verification state is truly terminal."
      );
    }
    ...
    if (
      ...
      const result = await completePlanVerificationWithSettlement({
        ...
        projectedSolutionCritiqueGaps:
          cachedRoundState.solutionCritique?.projected_gaps ?? [],
  ...
  async function runVerificationEnrichment(args: {
    ...
    return await runVerificationEnrichmentPass(
      ...
      {
        sessionId,
        selectedSpecialists: new Set(
          (args.selected_specialists ?? []).map((entry) => entry.trim()).filter(Boolean)
        ),
        includeFullContent: true,
        includeMessages: true,
        messageLimit: 5,
  ...
  async function runVerificationRound(args: {
    ...
    const planSnapshot = resultRecord.plan_snapshot as { artifact_id?: unknown } | undefined;
    let resultClassification =
      (result as { classification?: "complete" | "pending" | "infra_failure" }).classification ??
      "pending";
    const baseMergedGaps = Array.isArray(resultRecord.merged_gaps)
      ? resultRecord.merged_gaps
      : [];
    let solutionCritique: SolutionCritiqueRoundProjection | null = null;
    ...
    if (agentType === "ralphx-plan-verifier" && resultClassification === "complete") {
      try {
        solutionCritique = await generateSolutionCritiqueForRound({
          sessionId,
          targetArtifactId:
            typeof planSnapshot?.artifact_id === "string"
              ? planSnapshot.artifact_id
              : undefined,
        });
        mergedGaps = mergeVerificationGaps(
          baseMergedGaps,
          solutionCritique.projected_gaps
        );
        ...
        const requiredCriticSettlement =
          typeof resultRecord.required_critic_settlement === "object" &&
          resultRecord.required_critic_settlement !== null &&
          !Array.isArray(resultRecord.required_critic_settlement)
            ? (resultRecord.required_critic_settlement as Record<string, unknown>)
            : {};
    ...
    const requiredDelegates = Array.isArray((result as { required_delegates?: unknown }).required_delegates)
      ? ((result as { required_delegates: VerificationRoundDelegateInput[] }).required_delegates ?? [])
      : [];
    const createdAfter =
      typeof (result as { created_after?: unknown }).created_after === "string"
        ? ((result as { created_after: string }).created_after ?? "")
        : "";
