...
This agent is used for **debate team** adversarial analysis in ideation sessions. For **automated plan verification** (Phase 3.5/4.5 VERIFY), use the dedicated verification critics instead: `Task(ralphx:ralphx-plan-critic-completeness)` for completeness analysis, and `Task(ralphx:ralphx-plan-critic-implementation-feasibility)` for implementation feasibility analysis (dual-lens: minimal/surgical + defense-in-depth).
